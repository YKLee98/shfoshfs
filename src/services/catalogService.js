...
const BUNJANG_KPOP_COLLECTION_GID = 'gid://shopify/Collection/445888299257';
const BUNJANG_KIDULT_COLLECTION_GID = 'gid://shopify/Collection/446550966521';
...
function processCatalogRow(row, rowNumber) {
  ...
  const filterkpopCategoryIds = config.bunjang.filterkpopCategoryIds || [];
  if (filterkpopCategoryIds.length > 0 && product.categoryId && !filterkpopCategoryIds.includes(product.categoryId)) {
    logger.debug(`[CatalogSvc] Row #${rowNumber} PID ${product.pid} skipped: categoryId '${product.categoryId}' not in filter list [${filterkpopCategoryIds.join(', ')}].`);
    return null;
  }
  if (product.price < 0) {
...
async function syncBunjangProductToShopify(bunjangProduct, jobId = 'N/A') {
  ...
  try {
    ...
    if (operationType === 'update' && shopifyProductGid) {
      ...
      shopifyApiResult = await shopifyService.updateProduct(updateInput, BUNJANG_KPOP_COLLECTION_GID, null);
      ...
      shopifyApiResult = await shopifyService.createProduct(createInput, BUNJANG_KPOP_COLLECTION_GID, variantInfo);
      createdOrUpdatedProductId = shopifyApiResult?.id;
